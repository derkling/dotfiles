...
mailwidget = lain.widgets.maildir({
    timeout    = 60,
    mailpath   = os.getenv("HOME") .. "/Mail/Work",
    list_boxes = { "INBOX", "ML.lkml_sched", "ML.linaro_sched" }
})
...
bat_widget = lain.widgets.bat({
    settings = function()
        widget:set_markup(space3 .. "BAT " .. bat_now.time
...
bat_icon:set_image(beautiful.bat)
